...
-- The indicator window is transparent around the pill, so drop the frame.
-- no_initial_focus rather than no_focus: no_focus also stops mouse input, so
-- the pill could not be hovered, clicked, or dragged. The window itself refuses
-- keyboard focus, so dictated text still reaches the app you are typing in.
hl.window_rule({
  name = "wisper-indicator",
  match = { title = "^Wisper Clone Indicator$" },
  float = true, pin = true, no_initial_focus = true,
  border_size = 0, no_shadow = true, no_blur = true, opacity = "1 1",
})
-- The AppImage runs under XWayland and restores the spot you dragged the
-- indicator to. Native Wayland windows (pnpm tauri dev) can neither place
-- themselves nor refuse focus, so place them and keep focus out.
hl.window_rule({
  name = "wisper-indicator-wayland",
  match = { title = "^Wisper Clone Indicator$", xwayland = false },
  no_focus = true,
